...
public class BundleListActivity extends FragmentActivity {
	...
	static final int REAUEST_CODE_PICK_JAR = Activity.RESULT_FIRST_USER + 1;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (!TextUtils.isEmpty(path)) {
			try {
				...
				FrameworkHelper.getInstance(this).getFramework().getBundleContext().installBundle(path, new FileInputStream(new File(path)));
				updateUI();
	...
	public static class BundleList {
		...
		private void syncWithOsgi() {
			Framework f = FrameworkHelper.getInstance(null).getFramework();
